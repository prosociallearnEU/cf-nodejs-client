...
Apps.prototype.checkResources = function(token_type,access_token,zipResources){
    ...
    var qs = { };    
    var body = JSON.stringify(zipResources);

    //console.log(body);

    return this.REST.request2("PUT",url, headers, qs, body,"200");
}
...
Apps.prototype.uploadApp2 = function(token_type,access_token,appName,app_guid,dataRemoteFile,zipResources){
    ...
    return new Promise(function (resolve, reject) {
        request({method:'PUT', url:url, headers: headers, qs: qs, formData: form}, function (err, httpResponse, body) {
            if (err) {
                console.log(err);
                return reject(err);
            }
 ...
 * @return {[type]}                       [description]
 */
Apps.prototype.uploadApp3 = function(token_type,access_token,appName,app_guid,dataRemoteFile,zipResources){
    var url = this.API_URL + '/v2/apps/' + app_guid + '/bits'; 
    ...
    return new Promise(function (resolve, reject) {
        ...
        request({
          ...
          formData:{
           ...
           application:fs.createReadStream(ZIP_URL)
          },
          json:true
...
Stable upload process.
Trying to discover why Files are empty.

https://lists.cloudfoundry.org/archives/list/cf-dev@lists.cloudfoundry.org/thread/FEK2WNLB5EMQ645TAZYYANL6P5AXMVFJ/
...
index.html 0B
*/
Apps.prototype.uploadApp4 = function(token_type,access_token,appName,app_guid,dataRemoteFile,zipResources){
    var url = this.API_URL + '/v2/apps/' + app_guid + '/bits'; 
    var headers = {
        //'Accept': 'application/json',
        'Authorization': token_type + " " + access_token
